...
must_haves:
  ...
  key_links:
    ...
    - from: "lib/actions/dashboard.ts"
      ...
      via: "acknowledgeAlert updates data that getAttentionAlerts queries"
      pattern: "\\[ACK\\]"
---
...
<task type="auto">
  ...
  <files>lib/actions/dashboard.ts, lib/data/dashboard.ts</files>
  <action>
...
**Also update `getAttentionAlerts` in `lib/data/dashboard.ts`** to exclude acknowledged alerts:
- Add filter: `.not('error_message', 'like', '[ACK]%')` to the failed/bounced send_logs query.
- This ensures acknowledged alerts don't reappear in the alert list.

Error handling: try/catch, return `{ success: false, error }` on failure.
  </action>
  <verify>Run `pnpm typecheck` -- no errors. Verify retrySend and acknowledgeAlert are exported. Verify getAttentionAlerts in lib/data/dashboard.ts has the [ACK] exclusion filter.</verify>
  <done>Retry resets failed sends to pending for cron pickup. Acknowledge marks permanent failures as handled. getAttentionAlerts excludes [ACK] alerts.</done>
</task>
...
- getAttentionAlerts in lib/data/dashboard.ts filters out [ACK] alerts
</verification>
