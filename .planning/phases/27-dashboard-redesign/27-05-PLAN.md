...
<task type="auto">
  <name>Task 2: Update navigation (sidebar, bottom-nav, app-shell)</name>
  <files>
    components/layout/sidebar.tsx
    components/layout/bottom-nav.tsx
    components/layout/app-shell.tsx
...
2. Update `app/(dashboard)/layout.tsx` to fetch dashboard counts:
```typescript
...
```
Note: getDashboardCounts needs the business ID. Since the layout runs for all dashboard pages, we need to handle auth here. Import `getBusiness` from `@/lib/actions/business` and get business.id. If no business, pass 0.

Actually, since the layout needs the business_id and getDashboardCounts takes businessId, update getDashboardCounts to handle auth internally (get user -> get business -> run counts) like the other data functions in send-logs.ts. This matches the existing pattern where functions like `getMonthlyUsage()` handle auth internally.
  </action>
  <verify>Run `pnpm typecheck` -- no errors. Run `pnpm lint` -- no errors. Verify sidebar shows Dashboard as first item.</verify>
  <done>Dashboard link is first nav item with attention badge. Sidebar has "Add Job" button. Bottom nav updated for mobile. Badge count fetched in layout.</done>
</task>
