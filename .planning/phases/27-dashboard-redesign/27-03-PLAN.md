...
must_haves:
  ...
  artifacts:
    ...
    - path: "lib/types/dashboard.ts"
      provides: "QuickEnrollResult type export"
      exports: ["QuickEnrollResult"]
  key_links:
...
<task type="auto">
  ...
  <files>lib/actions/dashboard.ts, lib/types/dashboard.ts</files>
  <action>
Create `lib/actions/dashboard.ts` with 'use server' directive.

**`quickEnrollJob(jobId: string): Promise<QuickEnrollResult>`**

First, add and export the `QuickEnrollResult` type from `lib/types/dashboard.ts`:
```typescript
...
Then implement the server action in `lib/actions/dashboard.ts`:
...
Import `QuickEnrollResult` from `@/lib/types/dashboard` in the action file.
Follow existing server action patterns from `@/lib/actions/campaign.ts`.
  </action>
  <verify>Run `pnpm typecheck` -- no errors. Verify the action signature matches expected types. Verify QuickEnrollResult is exported from lib/types/dashboard.ts.</verify>
  <done>quickEnrollJob action finds matching campaign and creates enrollment, or signals no campaign exists. QuickEnrollResult type exported from lib/types/dashboard.ts.</done>
</task>
...
- QuickEnrollResult type exported from lib/types/dashboard.ts
...
- QuickEnrollResult type is explicitly exported for use by other modules
