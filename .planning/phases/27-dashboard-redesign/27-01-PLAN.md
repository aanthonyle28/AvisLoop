...
<task type="auto">
  ...
  <action>
Create `lib/data/dashboard.ts` with these functions. Use `createClient()` from `@/lib/supabase/server`.
...
This uses a two-step approach because Supabase client doesn't support NOT EXISTS subqueries well:
...
Step 3: Filter in JavaScript -- exclude jobs whose IDs appear in the enrolled set:
```typescript
...
Then for each remaining job, calculate `hoursElapsed`, `threshold`, and `isStale`:
- `hoursElapsed = differenceInHours(new Date(), new Date(job.completed_at))`
...
- Fetch two data sources in parallel:
  1. Failed/bounced send_logs: `status IN ('failed', 'bounced')` for this business, most recent 50. Exclude acknowledged alerts (where error_message starts with '[ACK]'). Map each to an AttentionAlert:
...
- This function handles auth internally (no businessId parameter). It follows the pattern used by functions like `getMonthlyUsage()` in `lib/data/send-logs.ts` that resolve the user and business internally.
- Implementation:
  1. Get authenticated user via `supabase.auth.getUser()`
  2. Get user's business from `businesses` table
  3. If no user or no business, return `{ readyToSend: 0, attentionAlerts: 0, total: 0 }`
  4. Count ready-to-send jobs (same two-step logic as getReadyToSendJobs but count only -- fetch completed job count, subtract enrolled job count)
  5. Count attention alerts: count failed/bounced send_logs (excluding '[ACK]' prefixed error_message) + count unresolved feedback
  6. Use `{ count: 'exact', head: true }` for efficiency where possible
  7. Return `{ readyToSend, attentionAlerts, total: readyToSend + attentionAlerts }`
- This is called from the dashboard layout to populate the nav badge, so it must be lightweight.
...
- Do NOT use 'use server' directive -- these are data functions, not server actions.
  ...
  <verify>Run `pnpm typecheck` -- no errors. Verify function signatures match the types from Task 1. Confirm getDashboardCounts() takes no parameters.</verify>
  <done>All four dashboard data functions exist, type-check clean, and follow existing codebase patterns. getDashboardCounts handles auth internally.</done>
</task>
...
- getDashboardCounts() takes no parameters (handles auth internally)
- getReadyToSendJobs uses explicit two-step approach (fetch completed, fetch enrolled, filter in JS)
...
- Ready-to-send correctly excludes jobs with active enrollments via two-step fetch-then-filter
- getDashboardCounts handles auth internally, callable without args from layout
