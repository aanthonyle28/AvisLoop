...
6. Update components/onboarding/steps/contact-step.tsx:
   - Rename file to customer-step.tsx
   - Rename component ContactStep -> CustomerStep
   - Update all internal references from contact to customer
   - Update imports in parent files that use this component

7. Update components/send/quick-send-tab.tsx:
   - Change all contact imports to customer imports: `import { getCustomers } from '@/lib/actions/customer'`
   - Rename local state/props: `contacts` -> `customers`, `selectedContacts` -> `selectedCustomers`
   - Update type annotations to use Customer instead of Contact
   - Update any UI text from "contact" to "customer"

8. Search for and update any other files that import from contacts:
   - Use grep to find: `from.*contacts` or `from.*contact`
   - Update each import to use new paths
  </action>
  <verify>
...
- components/onboarding/steps/customer-step.tsx exists with CustomerStep component
- components/send/quick-send-tab.tsx imports from customer.ts and uses Customer types
- `pnpm lint` passes
...
- [ ] app/(dashboard)/contacts/page.tsx redirects to /customers
- [ ] middleware.ts protects /customers route
- [ ] components/onboarding/steps/customer-step.tsx exists
- [ ] components/send/quick-send-tab.tsx uses Customer types and imports
- [ ] `pnpm typecheck` passes
- [ ] `pnpm lint` passes
</success_criteria>
