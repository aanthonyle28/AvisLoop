...
<task type="auto">
  <name>Task 1: Update CSV import dialog with phone parsing</name>
  <files>components/customers/csv-import-dialog.tsx</files>
  <action>
Update the CSV import dialog to parse phone numbers:
...
3. Update row parsing logic to include phone validation:
```typescript
...
5. Update import summary UI to show phone review:
```tsx
{results.phoneNeedsReview > 0 && (
...
6. Store phone_status when inserting customers:
```typescript
...
- "Review phone issues" button appears when needed
  ...
  <done>CSV import enhanced with phone parsing, phone_status tracking, and review prompt</done>
</task>
