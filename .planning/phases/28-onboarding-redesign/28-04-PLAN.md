...
Also update Step 1 (BusinessNameStep) to collect phone number in addition to business name. Add a phone input field below the business name input, using the same pattern. Update the formData to include phone:
```typescript
formData.append('phone', phone.trim())
```

And update Step 2 (GoogleReviewLinkStep) so that it calls `onGoToNext()` instead of `onComplete()` (since it's no longer the last step). Change the submit handler from `await onComplete()` to `onGoToNext()`, and update the Skip button similarly. Remove isSubmitting prop usage since step 2 is now mid-flow.
...
3. Fetch campaign presets for step 5:
```typescript
...
```
  ...
  <done>Step router handles cases 1-7, onboarding page clamps steps 1-7, presets fetched for step 5, Step 1 collects phone, Step 2 navigates forward instead of completing.</done>
</task>
...
2. `pnpm lint` passes
3. Navigating to /onboarding shows Step 1 with business name AND phone inputs
...
- Step 1 collects business name + phone number
