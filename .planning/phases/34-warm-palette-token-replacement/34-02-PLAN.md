...
must_haves:
  truths:
    ...
    - "No page has muddy, illegible, or cold-blue-looking areas in either mode (including AI diff highlight spans)"
...
<task type="auto">
  ...
  <action>
    ...
    **Pages to visit (in order of risk):**
    ...
    6. Check the AI preview diff (if accessible via campaign touch editing or AI personalization preview). Verify highlight spans use warm amber tint, not cold blue.

    **If visual issues are found:** Document each issue with a screenshot and specific description (which page, which element, what's wrong) in the SUMMARY file. These will be surfaced at the human-verify checkpoint for the user to evaluate. Do NOT silently skip issues — capture them all.
  </action>
  <verify>
    - `pnpm build` exits with code 0
    - All 10 pages render without console errors
    - Screenshots captured for light and dark mode
    - No visual issues identified (or issues documented in SUMMARY for the human-verify checkpoint)
  </verify>
  <done>
