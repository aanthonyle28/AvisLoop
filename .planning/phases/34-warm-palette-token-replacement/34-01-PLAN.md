...
<task type="auto">
  <name>Task 3: Update UI primitive focus/hover states from bg-accent to bg-muted</name>
  <files>
    components/ui/button.tsx
    components/ui/select.tsx
    components/ui/dropdown-menu.tsx
  ...
  <action>
    ...
    **Do NOT change any other classes in these files.** Only replace the accent-based interactive state classes identified above.
  </action>
  <verify>
    Run `pnpm typecheck` — must pass.
    Run `pnpm lint` — must pass.
  ...
  <done>
    All 4 UI primitive files use bg-muted or bg-secondary for interactive hover/focus states instead of bg-accent. The amber accent token is now purely decorative and will not appear on dropdown items, select items, button hovers, or dialog states.
  </done>
</task>
...
3. `grep "bg-accent" components/ui/button.tsx components/ui/select.tsx components/ui/dropdown-menu.tsx components/ui/dialog.tsx` returns zero hits
...
- UI primitive interactive states use bg-muted/bg-secondary instead of bg-accent
