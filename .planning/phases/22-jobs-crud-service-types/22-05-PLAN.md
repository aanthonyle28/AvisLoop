...
must_haves:
  ...
  artifacts:
    ...
    - path: "lib/data/business.ts"
      provides: "Data fetcher for service type settings"
      exports: ["getServiceTypeSettings"]
  ...
  key_links:
    ...
    - from: "app/(dashboard)/settings/page.tsx"
      to: "lib/data/business.ts"
      via: "getServiceTypeSettings import"
      pattern: "import.*getServiceTypeSettings.*from.*data/business"
      note: "Follows established pattern: lib/data/ for reads (getServiceTypeSettings), lib/actions/ for mutations (updateServiceTypeSettings). Matches existing getBusiness, getEmailTemplates in lib/data/business.ts."
---
...
<task type="auto">
  <name>Task 1: Add getServiceTypeSettings to lib/data/business.ts and updateServiceTypeSettings to lib/actions/business.ts</name>
  <files>lib/data/business.ts, lib/actions/business.ts</files>
  <action>
Add the getter function to lib/data/business.ts (following existing pattern where getBusiness and getEmailTemplates live):
...
Add the mutation action to lib/actions/business.ts:
...
Add the necessary imports if not present (revalidatePath from next/cache in actions file).
  </action>
  <verify>TypeScript compiles. Functions export correctly from their respective files.</verify>
  <done>lib/data/business.ts exports getServiceTypeSettings. lib/actions/business.ts exports updateServiceTypeSettings.</done>
</task>
