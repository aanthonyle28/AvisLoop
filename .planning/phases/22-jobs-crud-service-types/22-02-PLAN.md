...
must_haves:
  ...
  key_links:
    ...
    - from: "lib/data/jobs.ts"
      to: "lib/actions/job.ts"
      via: "intentional separation"
      note: "Data layer (lib/data/) handles reads; action layer (lib/actions/) handles mutations. This matches existing lib/data/business.ts pattern."
---
...
<task type="auto">
  ...
  <action>
Create lib/data/jobs.ts with data fetching functions following lib/data/business.ts pattern.

Note: This follows the established pattern where lib/data/ handles reads (getBusiness, getEmailTemplates) and lib/actions/ handles mutations (createJob, updateJob). This separation keeps the data layer clean and reusable in Server Components.
