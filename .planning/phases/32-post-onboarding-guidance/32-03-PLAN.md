...
files_modified:
  ...
  - components/campaigns/campaigns-client.tsx
...
must_haves:
  truths:
    ...
    - "Checklist hidden if user has dismissed it"
    - "Checklist auto-collapses after 3 days (header only, expandable)"
    - "Jobs page shows tooltip hint on first visit pointing at Add Job button"
    - "Campaigns page shows tooltip hint on first visit"
...
- Includes collapsed/firstSeenAt for auto-collapse after 3 days
  </action>
  <verify>
...
<task type="auto">
  <name>Task 6: Add first-visit hint to Campaigns page</name>
  <files>
    components/campaigns/campaigns-client.tsx
  </files>
  <action>
Update `components/campaigns/campaigns-client.tsx` to add a FirstVisitHint on the campaigns page:
...
2. Find the campaign list or first campaign card area.

3. Wrap the first campaign card (or the section header) with FirstVisitHint:
```tsx
<FirstVisitHint
  ...
  side="bottom"
>
  {/* First campaign card or section header */}
</FirstVisitHint>
```

Alternative: If wrapping a card doesn't make sense, add the hint to the page header area pointing at the campaign list.

Key points:
- Hint only shows on first visit to Campaigns page
- Reinforces that campaigns are automatic (V2)
- Encourages user to review/verify settings
- Side="bottom" or "right" depending on layout
  </action>
  <verify>
    `pnpm typecheck` passes, `pnpm lint` passes
  </verify>
  <done>
    FirstVisitHint added to Campaigns page with V2-aligned messaging about automation
  </done>
</task>
...
8. Checklist auto-collapses after 3 days
9. Jobs page imports and renders FirstVisitHint around Add Job button
10. Campaigns page imports and renders FirstVisitHint
11. Hints show on first visit, not on subsequent visits
12. V2-aligned messaging in hints and checklist
</verification>
...
5. Checklist auto-collapses after 3 days (shows header only)
6. Checklist fetched in parallel with other dashboard data
7. Jobs page shows tooltip hint on first visit
8. Campaigns page shows tooltip hint on first visit
9. Hints dismissible via X or "Got it" button
10. Hints tracked in localStorage (show once)
11. All messaging reinforces V2 workflow (jobs primary, automation)
12. Build and typecheck pass
</success_criteria>
