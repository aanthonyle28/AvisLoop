...
export const CHECKLIST_ITEMS = [
  ...
  {
    // NOTE: This tracks customer_feedback count as a proxy for customer engagement.
    // customer_feedback stores private feedback (1-3 star ratings), not Google reviews.
    // We don't currently track actual Google review submissions.
    // TODO: Consider renaming or tracking actual review clicks when analytics are added.
    id: 'first_response' as const,
    title: 'Get first customer response',
    description: 'Watch your automation work - customers will respond',
    href: '/analytics',
    cta: 'View Analytics',
  },
] as const
...
export async function getChecklistState(businessId: string): Promise<{
  ...
  const [jobsResult, completedJobsResult, campaignsResult, feedbackResult, businessResult] = await Promise.all([
    ...
    // Count customer responses (from customer_feedback - includes private feedback)
    // This is a proxy for customer engagement, not actual Google reviews
    supabase
  ...
  const items: Record<ChecklistItemId, boolean> = {
    ...
    first_response: feedbackCount > 0, // Any feedback counts as customer engagement
  }
...
- Uses feedback count as proxy for customer engagement (documented limitation)
 ...
 * - Add job -> Set up campaign -> Complete job -> Get response
 *
 * Auto-hides when:
