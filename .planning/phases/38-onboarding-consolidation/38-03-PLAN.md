...
<task type="auto">
  ...
  **In `setup-progress-drawer.tsx`:**
  ...
  <verify>Run `pnpm typecheck`. Grep `setup-progress-pill.tsx` for `bg-primary/10` — should be zero hits (the specific token being replaced). Grep `setup-progress-pill.tsx` for `bg-warning-bg` — should exist. Grep `setup-progress-drawer.tsx` for `bg-warning` — should find the progress bar line.</verify>
...
<task type="auto">
  ...
  **Important: Use `await` only, NOT `void`.** The research file shows `void markCampaignReviewed()` as an alternative — that is incorrect and must be ignored. `await` guarantees the flag is written before the page renders. Without `await`, a cold-start timeout could silently skip the write and the checklist item would stay incomplete forever. The action short-circuits immediately on subsequent visits (one read, no write), so it adds minimal latency.
  </action>
  <verify>
