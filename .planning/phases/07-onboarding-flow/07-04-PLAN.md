...
2. Update OnboardingWizard children to render actual step components:
   ...
   <OnboardingWizard initialStep={currentStep} ...>
     {({ draftData, setDraftData, goToNext, goToStep }) => {
       switch (currentStep) {
         case 1:
           return <BusinessStep onComplete={goToNext} defaultValues={...} />
         case 2:
           return <ContactStep onComplete={goToNext} onSkip={goToNext} businessId={...} />
         case 3:
           return <SendStep contact={...} business={...} template={...} onComplete={...} />
       }
     }}
   </OnboardingWizard>
   ```
...
3. Pass appropriate data to each step:
   - BusinessStep: existing business name/review_link
   - ContactStep: businessId for creating contact
   - SendStep: first contact (if exists), business, default template
  </action>
  <verify>
`pnpm typecheck` passes
`pnpm lint` passes
