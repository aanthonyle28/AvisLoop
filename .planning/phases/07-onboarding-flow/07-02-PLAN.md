...
export function OnboardingWizard({
  ...
  initialStep: number
  children: (props: { draftData: Record<string, any>; setDraftData: (key: string, value: any) => void }) => ReactNode
...
2. Navigation functions:
   ...
   - handleSkip() - advances to next step if current is skippable
...
Note: Step content components will be added in 07-03. For now, render placeholder content in the wizard children.
...
<OnboardingWizard initialStep={currentStep}>
  {({ draftData, setDraftData }) => (
    <div className="p-6 text-center text-muted-foreground">
      Step {currentStep} content (placeholder for 07-03)
    </div>
  )}
</OnboardingWizard>
```
