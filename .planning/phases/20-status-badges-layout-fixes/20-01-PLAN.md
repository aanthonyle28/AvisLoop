...
must_haves:
  truths:
    - "Every status badge across the app (history page, send page, drawers, recent activity strip, scheduled table) renders with identical Figma-spec colors, icons, and typography"
...
<task type="auto">
  <name>Task 3: Verify existing StatusBadge consumers inherit new styling</name>
  <files>components/history/history-columns.tsx, components/history/request-detail-drawer.tsx, components/send/recent-activity-strip.tsx</files>
  <action>
These three files already import and render StatusBadge. They are NOT modified by this plan -- they automatically inherit the rebuilt styling from Task 1. This task verifies that the import paths and usage are still valid after the rebuild:

1. Confirm `components/history/history-columns.tsx` imports `StatusBadge` from `./status-badge` and passes a `status` prop.
2. Confirm `components/history/request-detail-drawer.tsx` imports `StatusBadge` from `./status-badge` and passes a `status` prop.
3. Confirm `components/send/recent-activity-strip.tsx` imports `StatusBadge` from `@/components/history/status-badge` and passes a `status` prop.
4. Confirm the `SendStatus` type export from Task 1 is compatible with how these consumers cast their status values (all use `as SendStatus` or `as string`).

No code changes are needed in these files. If typecheck passes with no errors in these files, the verification is complete.
  </action>
  <verify>
Run `pnpm typecheck` -- no errors in history-columns.tsx, request-detail-drawer.tsx, or recent-activity-strip.tsx. Grep for `import.*StatusBadge` across the codebase confirms all 4 consumers (including scheduled-table from Task 2) import from status-badge.
  </verify>
  <done>
All existing StatusBadge consumers (history-columns, request-detail-drawer, recent-activity-strip) correctly inherit the rebuilt Figma-spec styling without any code changes. The contacts page is confirmed to not use send status badges (it shows contact status, not send status).
  </done>
</task>
