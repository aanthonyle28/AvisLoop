...
1. Query send_logs schema:
   ```sql
   SELECT column_name, data_type
   FROM information_schema.columns
   WHERE table_name = 'send_logs'
   AND column_name IN ('channel', 'provider_message_id', 'customer_id');
   ```
   Expected: All three columns exist

2. Query sms_retry_queue:
   ```sql
   SELECT routine_name FROM information_schema.routines
   WHERE routine_name LIKE '%sms_retries%';
   ```
   Expected: claim_due_sms_retries, recover_stuck_sms_retries
...
**Webhook verification (if possible):**
1. Check /api/webhooks/twilio/inbound route exists
2. Check /api/webhooks/twilio/status route exists
3. Verify routes return 403 without signature header:
   ```bash
...
2. Verify SMS received with opt-out footer
3. Reply STOP
