...
must_haves:
  truths:
    ...
    - "SMS sending checks consent and quiet hours before sending"
    - "SMS fallback to email when customer has no phone"
  artifacts:
...
<task type="auto">
  <name>Task 3: Create channel selector component and update send page</name>
  <files>components/send/channel-selector.tsx, app/dashboard/send/page.tsx</files>
  <action>
...
The send page needs to be updated to:
...
Look at the existing send page/component structure and:
- Add channel state
- Add ChannelSelector component
- Conditionally show character counter
- Switch between sendReviewRequest and sendSmsRequest based on channel
- Update form submission to include channel-specific fields

Key integration points:
- When customer selected, check phone and consent to determine SMS availability
- When channel is SMS, show character counter instead of subject field
- When SMS queued for quiet hours, show toast with scheduled time
  </action>
  <verify>
Run `pnpm typecheck` - no errors
Run `pnpm lint` - no errors
Navigate to /dashboard/send - channel selector visible
Select customer with phone - SMS tab enabled
Select customer without phone - SMS tab disabled with reason
  </verify>
  <done>
Channel selector allows email/SMS toggle with disabled state for customers lacking phone or consent. Send page updated to switch between email and SMS actions.
  </done>
</task>
