...
<objective>
Complete icon migration from lucide-react to @phosphor-icons/react. QA-FIX migrated 11 high-priority files; this plan migrates the remaining ~22 application component files.
...
<task type="auto">
  <name>Task 2: Migrate job, send, and settings components (4 files)</name>
  <files>
    components/jobs/add-job-sheet.tsx
    components/jobs/edit-job-sheet.tsx
    components/send/quick-send-tab.tsx
  ...
  <action>
Apply same icon mappings as Task 1. Note that add-job-sheet.tsx may already be partially migrated from Plan 03.
...
For add-job-sheet.tsx and edit-job-sheet.tsx, ensure:
...
2. `pnpm lint` passes
3. `grep -r "from 'lucide-react'" components/` returns only UI primitives (dialog, sheet, select, dropdown, sonner)
4. All migrated components import from '@phosphor-icons/react'
5. Icons render correctly in UI (spot check key pages)
</verification>
