...
<objective>
Complete icon migration from lucide-react to @phosphor-icons/react. QA-FIX migrated 11 high-priority files; this plan migrates the remaining 13 application component files identified via grep.

Purpose: ICON-01 requirement. Consistent icon system across the codebase.
Output: All 13 application components using Phosphor icons.

Note: Grep results show 13 files with lucide imports (not 21 as originally listed). Excludes UI primitives (dialog, sheet, select, dropdown, checkbox, sonner) which are acceptable per 30-RESEARCH.md.
</objective>
...
<task type="auto">
  <name>Task 1: Migrate customer components (5 files)</name>
  <files>
    components/customers/add-customer-sheet.tsx
    components/customers/csv-preview-table.tsx
    components/customers/customer-columns.tsx
...
```
  ...
  <done>All 5 customer component files using Phosphor icons</done>
</task>
...
<task type="auto">
  <name>Task 4: Migrate marketing component (1 file)</name>
  <files>
    components/marketing/v2/animated-demo.tsx
  </files>
  <action>
Apply icon mappings for marketing component:
| Lucide | Phosphor |
|--------|----------|
...
For animated-demo.tsx, ensure any animation classes are preserved:
```typescript
// Keep motion classes
...
```
  ...
  <verify>Run `pnpm typecheck` after changes</verify>
  <done>Marketing component using Phosphor icons</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Icon migration complete across 13 application component files</what-built>
  <how-to-verify>
...
3. Test icon interactions:
   ...
   - Marketing page icons and animations
...
4. All 13 migrated components import from '@phosphor-icons/react'
...
<success_criteria>
- All 13 application components migrated to Phosphor (ICON-01)
