...
<task type="auto">
  ...
  <action>
    ...
    - Build PRICE_TO_TIER inside POST handler, NOT at module level (env vars may be undefined at load time)
  </action>
  <verify>`pnpm typecheck` passes</verify>
  <done>Webhook endpoint with signature verification and dynamic price mapping</done>
</task>
...
- [ ] PRICE_TO_TIER is built inside POST handler (not at module level)
