...
<objective>
Create 3 dashboard onboarding cards (create contact, create template, send test), implement auto-completion tracking, and add is_test exclusion to quota counting. Replace the old onboarding checklist.
...
Output: OnboardingCards component, updated onboarding data layer, quota exclusion for test sends, dashboard integration.
</objective>
...
<task type="auto">
  <name>Task 1: Onboarding card completion tracking + test send quota exclusion</name>
  <files>lib/data/onboarding.ts, lib/actions/onboarding.ts, lib/data/send-logs.ts</files>
  <action>
...
4. Search for other quota queries and confirm they also filter is_test
5. Run `pnpm typecheck`
  </verify>
  <done>Card status auto-detected from DB, is_test sends excluded from quota, markOnboardingCardStep action available for manual marking.</done>
</task>
...
6. Remove the old `getOnboardingStatus` import if it's only used for the redirect check. Actually, keep it -- it's still used for the redirect check at the top of the page.
  ...
  <verify>
1. Read onboarding-cards.tsx and confirm 3 cards with correct config
...
5. Run `pnpm typecheck` and `pnpm lint`
  ...
  <done>Dashboard shows 3 numbered onboarding cards after wizard completion, cards auto-detect completion from DB, cards disappear when all 3 done, old checklist deprecated.</done>
</task>
...
5. Test sends excluded from monthly quota counting (is_test=false filter)
6. `pnpm typecheck` and `pnpm lint` pass
</verification>

<success_criteria>
- 3 onboarding cards visible on dashboard after wizard completion
- Cards navigate to /contacts, /dashboard/settings, /send respectively
