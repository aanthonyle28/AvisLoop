...
depends_on: []
files_modified:
  - supabase/migrations/00016_onboarding_redesign.sql
...
must_haves:
  ...
  artifacts:
    - path: "supabase/migrations/00016_onboarding_redesign.sql"
...
<task type="auto">
  ...
  <files>supabase/migrations/00016_onboarding_redesign.sql</files>
  <action>
...
Do NOT recreate the onboarding_steps_completed column if it already exists. Only update the default value.
  ...
  <verify>Read the migration file and confirm it has: is_test column, partial index, default update for onboarding_steps_completed.</verify>
  <done>Migration file exists with is_test boolean on send_logs, partial index for quota queries, and updated default for onboarding_steps_completed.</done>
</task>
...
<verification>
1. Migration SQL file has correct ALTER TABLE statements for is_test and onboarding_steps_completed default
...
<success_criteria>
- Database migration ready with is_test column + partial index + JSONB default update
