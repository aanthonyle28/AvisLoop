...
must_haves:
  truths:
    - "Every dashboard route has a loading.tsx with skeleton screens"
    - "Skeleton screens have explicit dimensions to prevent layout shift"
    - "Empty states follow icon + text + CTA format"
    - "Loading skeletons match the structure of the loaded content"
  artifacts:
...
export function TableSkeleton({
  ...
  return (
    <div className="rounded-lg border bg-card" style={{ minHeight: `${rows * 73 + (showHeader ? 48 : 0)}px` }}>
      {/* Header */}
      {showHeader && (
...
5. No layout shift when content loads (skeletons have matching dimensions)
</verification>
...
- Skeleton screens prevent layout shift with explicit dimensions
