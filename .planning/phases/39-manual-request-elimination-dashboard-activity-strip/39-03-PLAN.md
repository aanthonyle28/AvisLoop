...
<task type="auto">
  ...
  <files>app/(dashboard)/campaigns/page.tsx, components/campaigns/campaigns-page-client.tsx, components/customers/customers-client.tsx</files>
  <action>
...
4. Render the `QuickSendModal` at the end of the component JSX (before the closing fragment):
   ...
   - Check the `CustomersClient` props interface and the parent page (`app/(dashboard)/customers/page.tsx`).
   - The parent page will need to fetch and pass these additional props.
   - In `app/(dashboard)/customers/page.tsx`, add:
     ```typescript
     import { getBusiness } from '@/lib/actions/business'
     import { getMonthlyUsage } from '@/lib/data/send-logs'
     ```
   - Fetch business and monthlyUsage alongside existing queries.
   - Pass `business`, `templates` (from `business.message_templates` filtered to email), `monthlyUsage`, and `hasReviewLink` down to `CustomersClient`.
   ...
   In the parent page (`app/(dashboard)/customers/page.tsx`), add these to the server-side fetching and pass them as props.

**Files that also need modification** (add to the task scope):
- `app/(dashboard)/customers/page.tsx` -- add business + monthlyUsage fetching and pass to CustomersClient
