...
2. Run 3 queries in parallel (Promise.all):
...
3. After all 3 queries return, merge results into a single array.
...
```
  ...
  <done>getRecentCampaignEvents is exported from lib/data/dashboard.ts, returns CampaignEvent[], queries send_logs (campaign-attributed), customer_feedback, and campaign_enrollments</done>
</task>
...
6. Existing `DashboardKPIs` interface is unchanged (still has all 6 fields)
7. Existing `getDashboardKPIs` function is unchanged
</verification>
...
- getRecentCampaignEvents queries 3 tables, merges, sorts by timestamp, returns top N
