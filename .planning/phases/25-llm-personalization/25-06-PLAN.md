...
must_haves:
  truths:
    ...
    - "MVP uses estimation for personalization rate (documented as temporary)"
  artifacts:
...
export interface PersonalizationStats {
  ...
  isEstimated: boolean  // MVP flag - true until actual tracking is implemented
}
 ...
 * Currently estimates personalization rate based on typical success rate.
 * This is documented as temporary until we add actual tracking.
 *
 * Future implementation options:
 * 1. Query Resend tags via API for 'personalized: true/false' counts
 * 2. Add 'personalized' boolean column to send_logs table
 * 3. Parse existing send_logs metadata if stored
 *
 * For now, we use estimation because:
 * - Resend tag querying requires additional API calls and may hit rate limits
 * - Adding a column requires migration coordination with 25-07
 * - Estimation matches expected behavior from CONTEXT.md ("97% personalized, 3% fallback")
 */
export async function getPersonalizationStats(
  ...
  // MVP: Estimate personalization rate
  // Based on CONTEXT.md: "97% personalized, 3% standard template"
  // Using 95% as conservative estimate accounting for edge cases
  //
  // TODO: Replace with actual tracking in future iteration:
  // - Option A: Add 'personalized' column to send_logs (recommended)
  // - Option B: Query Resend API for tag statistics
  const ESTIMATED_PERSONALIZATION_RATE = 95
  const personalizedCount = Math.round(totalSends * (ESTIMATED_PERSONALIZATION_RATE / 100))
  ...
  return {
    ...
    personalizedCount,
    fallbackCount,
    personalizationRate: totalSends > 0 ? ESTIMATED_PERSONALIZATION_RATE : 0,
    period,
    isEstimated: true,  // Flag for UI to indicate estimation
  }
}
...
- MVP uses estimation with `isEstimated: true` flag for transparency
...
- TODO comments document future implementation path
  </action>
  <verify>
TypeScript compiles: `pnpm typecheck` passes.
File exports getPersonalizationStats, getLLMUsageStats.
PersonalizationStats interface includes isEstimated boolean.
  </verify>
  <done>Personalization stats tracked and queryable from send_logs with documented MVP estimation</done>
</task>
    ...
    <div className="flex items-center justify-between">
      <div>
        ...
        <div className="text-xs text-muted-foreground">
          Last 7 days{stats.isEstimated && ' (estimated)'}
        </div>
      </div>
...
- Show "(estimated)" indicator when stats.isEstimated is true for transparency
  </action>
  <verify>
TypeScript compiles: `pnpm typecheck` passes.
Settings page includes personalization section with stats.
Estimated stats show "(estimated)" indicator.
  </verify>
  <done>Settings page shows personalization health and usage stats with estimation indicator</done>
</task>
...
- PersonalizationStats includes isEstimated boolean
- Campaign form has personalization toggle in advanced settings
- Settings page shows personalization section with "(estimated)" when applicable
</verification>

<success_criteria>
- getPersonalizationStats returns weekly stats from send_logs with isEstimated flag
- Campaign form has "AI Personalization" toggle (default on)
- Settings page shows personalization rate, send count, and usage bar
- Estimated stats clearly indicated in UI with "(estimated)" label
