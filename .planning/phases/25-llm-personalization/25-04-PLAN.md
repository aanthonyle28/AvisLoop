...
if (personalizationResult.personalized) {
  ...
  // Store personalized body for future use
  // For now, still use ReviewRequestEmail component
  // TODO: Use personalized body directly in Phase 25 enhancement
}

// Render email (still using component for now)
...
- Subject can be personalized, body rendered by component (for now)
...
Cron route imports personalizeWithFallback from lib/ai/fallback.
  ...
  <done>Campaign touch processor uses LLM personalization with fallback to template</done>
</task>
